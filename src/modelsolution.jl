...
function convert_to_real(x, flag)
    flag = flag || maximum(abs.(imag.(x))) .> 1e-5
    x = real.(x)
    return x, flag
end
convert_to_real(x) = convert_to_real(x, true)[1]
...
function solve_model(A, B, C, Σ, n, m)
    ...
    n̄ = count(Istable)
    m̄ = count(.!Istable)
    (m̄ < m) && return empty_solution() # infinite solutions
    (m̄ > m) && return empty_solution() # no stationary solution
    ...
    flag_complex = false
    P, flag_complex = convert_to_real(P, flag_complex)
    Q, flag_complex = convert_to_real(Q, flag_complex)
...
function Model(A, B, C, Σ, n)
    m = size(A, 1) - n
    q = size(Σ, 1)
    A, B, C, Σ = map(convert_to_matrix, (A, B, C, Σ))
    S, flag_rank, flag_complex = solve_model(A, B, C, Σ, n, m)
